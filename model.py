...
class Decoder(nn.Module):
    ...
    def Attention(self, hidden, encoder_outputs, encoder_maskings):
        ...
        hidden = hidden.squeeze(0).unsqueeze(2)  # Hidden: (1, batch, dimension)-> (batch, dimension, 1)==히든 : (1,배치,차원) -> (배치,차원,1)
        
        batch_size = encoder_outputs.size(0) # B
        max_len = encoder_outputs.size(1) # T
        energies = self.attn(encoder_outputs.contiguous().view(batch_size*max_len,-1)) # B*T,D -> B*T,D
        energies = energies.view(batch_size,max_len,-1) # B,T,D (Batch, time, dimension)(배치,타임,차원)
    ...
    def forward(self, input,context,encoder_outputs,encoder_maskings,training=True):
        ...
        for i in range(length): # Because input_sequence and output_sequence have the same length. ==Input_sequence와 Output_sequence의 길이가 같기 때문..
            ...
            # Then calculate the Context Vector as Attention == 그 다음 Context Vector를 Attention으로 계산
            context = self.Attention(hidden[0], encoder_outputs,encoder_maskings) 
        # Yogo Caution! After column-wise concating the time-step, reshape !! == 요고 주의! time-step을 column-wise concat한 후, reshape!!
